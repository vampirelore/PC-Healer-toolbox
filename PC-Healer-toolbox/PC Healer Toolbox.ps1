...
function Start-Installer {
    ...
    if (![string]::IsNullOrWhiteSpace($command)) {
        Start-Process powershell.exe -ArgumentList '-NoExit','-Command', $command -Verb RunAs -Wait
    } else {
        [System.Windows.Forms.MessageBox]::Show("Installer command was empty or null.", "Error", "OK", "Error")
    }
}
...
function Install-Software {
    ...
    if (Get-Command winget -ErrorAction SilentlyContinue) {
        Start-Installer $wingetCmd
    } elseif (Get-Command choco -ErrorAction SilentlyContinue) {
        Start-Installer $chocoCmd
    } else {
        $msg = "Neither Winget nor Chocolatey is installed.`r`nPlease install one to proceed with software installs."
        [System.Windows.Forms.MessageBox]::Show($msg, "Missing Package Managers", "OK", "Error")
    }
}
...
$lblUtilities.Text = "Choose Your Tweaks:"
$lblUtilities.Location = '10,10'
$lblUtilities.Size = '200,20'
$tabUtilities.Controls.Add($lblUtilities)
...
$btnRunTweaks.Add_Click({
    ...
    if ($chkTelemetry.Checked -or $chkTrack.Checked) {
        try {
            ...
            $outputUtilities.AppendText("Cortana and telemetry service disabled.`r`n")
        } catch {
            $outputUtilities.AppendText("Failed to disable Cortana or telemetry -
    ...
    if ($chkEdge.Checked) {
        try {
            Get-AppxPackage -AllUsers | Where-Object {
                $_.Name -like "*Microsoft.MicrosoftEdge*" -and $_.IsFramework -ne $true
            } | ForEach-Object {
                try {
                    Remove-AppxPackage -Package $_.PackageFullName -ErrorAction Stop
                    $outputUtilities.AppendText("Removed Edge app: $($_.Name)`r`n")
                } catch {
                    $outputUtilities.AppendText("Skipped Edge app: $($_.Name) - $($_.Exception.Message)`r`n")
                }
            }
        } catch {
            $outputUtilities.AppendText("Error while processing Edge apps - $($_.Exception.Message)`r`n")
        }
    }
    

    if ($chkBakgrnd.Checked) {
        $outputUtilities.AppendText("Processing background apps...`r`n")
        ...
        Get-AppxPackage -AllUsers | Where-Object {
            $name = $_.Name
            ($excluded -notcontains $name) -and ($_.IsFramework -ne $true)
        } | ForEach-Object {
            try {
                Remove-AppxPackage -Package $_.PackageFullName -ErrorAction Stop
                $outputUtilities.AppendText("Removed: $($_.Name)`r`n")
            } catch {
                $outputUtilities.AppendText("Skipped: $($_.Name) - $($_.Exception.Message)`r`n")
            }
        }
    
        # Registry tweak to stop background access
        ...
        $outputUtilities.AppendText("Background app policy set via registry.`r`n")
    }
...
try {
    ...
    Set-ItemProperty -Path $searchPath -Name "AllowCortana" -Value 0 -Force
    # ConnectedUserExperience is not a valid property here - this may be a confusion with another service
    # Instead, you should disable the Connected User Experience service like this:
    ...
    $outputUtilities.AppendText("Failed to disable Cortana or related services - $($_.Exception.Message)`r`n")
}
...
$btnInstall.Add_Click({
    ...
    if ($chkBrowser.Checked) {
        ...
        if ($response -eq "Yes") {
            Install-Software `
                'winget install --id=Microsoft.DesktopAppInstaller -e --accept-package-agreements --accept-source-agreements' `
                'choco install ungoogled-chromium -y'
            $outputUtilities.AppendText("Chromium Browser installer launched.`r`n")
        } else {
    ...
    if ($chkWinget.Checked) {
        $outputUtilities.AppendText("Attempting to install Winget...`r`n")
        Install-Software `
            'winget install --id=Microsoft.Winget.Source -e --accept-package-agreements --accept-source-agreements' `
            'choco install winget -y'
        $outputUtilities.AppendText("Winget installer command issued.`r`n")
    }

    if ($chkMal.Checked) {
        $outputUtilities.AppendText("Attempting to install Malwarebytes...`r`n")
        Install-Software `
            'winget install --id=Malwarebytes.Malwarebytes -e --accept-package-agreements --accept-source-agreements' `
            'choco install malwarebytes -y'
        $outputUtilities.AppendText("Malwarebytes installer command issued.`r`n")
    }

    if ($chkPtoys.Checked) {
        $outputUtilities.AppendText("Attempting to install PowerToys...`r`n")
        Install-Software `
            'winget install --id=Microsoft.PowerToys -e --accept-package-agreements --accept-source-agreements' `
            'choco install powertoys -y'
        $outputUtilities.AppendText("PowerToys installation command issued.`r`n")
    }

    if ($chkCPUZ.Checked) {
        $outputUtilities.AppendText("Attempting to install CPU-Z...`r`n")
        Install-Software `
            'winget install --id=CPUID.CPUID -e --accept-package-agreements --accept-source-agreements' `
            'choco install cpu-z -y'
        $outputUtilities.AppendText("CPU-Z installer command issued.`r`n")
    }

    if ($chkCCleaner.Checked) {
        $outputUtilities.AppendText("Attempting to install CCleaner...`r`n")
        Install-Software `
            'winget install --id=Piriform.CCleaner -e --accept-package-agreements --accept-source-agreements' `
            'choco install ccleaner -y'
        $outputUtilities.AppendText("CCleaner installer command issued.`r`n")
    }
...
$outputDev.AppendText("VS Code installer command issued.rn")
} else {
$outputDev.AppendText("VS Code installation canceled.rn")
}
})
...
$btnGit.Add_Click({
    ...
    if ($response -eq "Yes") {
        Install-Software `
            ...
            'choco install git -y'
        $outputDev.AppendText("Git installer command issued.`r`n")
    } else {
